...
Claude Codeは、すべてのプロジェクトで一貫した開発標準を遵守します。以下はクロスプロジェクトで適用される標準です。

### Python環境とツーチェーン

#### 環境管理

Python環境の作成と管理には **`uv`** を使用してください。
...
**理由**: uvはRustで実装された高速なPythonパッケージマネージャー。pip/venvより性能が優れており、プロジェクト間で一貫性があります。

#### Linter とFormatter

Pythonコードの品質管理には **`ruff`** を使用してください。
...
**理由**: ruffはRustで実装された超高速なツール。pylint、black、isortなどの機能を統一されたツールで提供し、セットアップと保守が簡潔です。

### Docker

#### コマンド形式

Docker Compose コマンドは新しい形式を使用してください。
...
**理由**: `docker compose` はDocker CLIの公式統合コマンド。`docker-compose`スタンドアロンツールは保守終了に向かっています。
