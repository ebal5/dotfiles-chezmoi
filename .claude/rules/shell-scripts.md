...
シェルスクリプトを編集した後は、必ず以下のlintを実行すること:

1. `shfmt -i 2 -ci -w <edited-file>` - 編集したファイルをフォーマット
2. `shellcheck <edited-file>` - 編集したファイルを静的解析

`.tmpl`ファイルの場合はlintをスキップしてよい（上記の制限事項を参照）。
